...
const UserProfileLeft = props => {
  const { userSidebar, setUserSidebar, profileUserData, setUserStatus, isBelowLgScreen, isBelowSmScreen } = props
  const [twoStepVerification, setTwoStepVerification] = useState(true)
  const [notification, setNotification] = useState(false)

  if (!profileUserData) return null
          ...
          <div className='flex flex-col gap-6 p-6 pbs-3'>
            ...
            <div className='flex flex-col gap-1'>
              ...
              <List className='plb-0'>
                <ListItem
                  disablePadding
                  secondaryAction={<Switch checked={twoStepVerification} onChange={() => setTwoStepVerification(prev => !prev)} />}
                >
                  <ListItemButton onClick={() => setTwoStepVerification(prev => !prev)} className='p-2'>
                    <ListItemIcon>
                ...
                <ListItem
                  disablePadding
                  secondaryAction={<Switch checked={notification} onChange={() => setNotification(prev => !prev)} />}
                >
                  <ListItemButton onClick={() => setNotification(prev => !prev)} className='p-2'>
                    <ListItemIcon>
